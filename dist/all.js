...
require(['config'], function() { //config：加载配置文件
	require(['jquery', 'jqcookie'], function() { //加载配置里面的内容,引入jquery和cookie插件
		...
		(function() {
			...
			$.ajax({
				url: 'http://127.0.0.1/Amazon/php/cart.php',
				dataType: 'json'
			}).done(function(d) {
				var $moreGoogHtml = '';
...
require(['config'], function() { //config：加载配置文件
	require(['jquery', 'jqcookie'], function() { //加载配置里面的内容,引入jquery和cookie插件
		...
		$.ajax({
			url: 'http://127.0.0.1/Amazon/php/index.php',
			dataType: 'json'
		}).done(function(d) {
			...
			(function() {
				...
				$(window).scroll(function() {
					...
					$(".build .floor").each(function() {
						...
						var $top = $(".build .floor").eq($(this).index('.floor')).offset().top + 280
						if($top > $scrollTop) {
	...
	require(['jquery', 'jqcookie'], function() { //加载配置里面的内容,引入jquery和cookie插件
		...
		(function() {
			...
			$('.sub').click(function() {
				$.ajax({
					type: 'post',
					url: 'http://127.0.0.1/Amazon/php/login.php',
					data: {
						name: $('#username').val(),
						pass: $('#password').val()
					},
					success: function(data) {
						if(!data) {
							...
							$('.pswBox span').html('');
							$(window).attr('location','http://127.0.0.1/Amazon/index.html');
						}
					},
					error: function(e) {
						alert(e)
			...
			$('#username').blur(function() {
				...
				if($(this).val() != '') {
					if($reg.test($(this).val())) {
						$.ajax({
							type: 'post',
							url: 'http://127.0.0.1/Amazon/php/registor.php',
							data: 'name=' + $(this).val(),
							success: function(data) {
								if(data) {
									$('.usernameBox span').html('用户名已经存在');
